...
public macro ErrorCode(codeLength: Int? = nil, delimiter: StaticString? = nil, codeCharacters: StaticString? = nil) = #externalMacro(module: "ErrorCodeMacros", type: "ErrorCodeMacro")

/**
 Adds a pseudo-random, alphanumeric ``ErrorCode/ErrorCode/opaqueCode`` `String` property to each member of an `enum` declaration that is safe to present to a user without unintentionally exposing implementation details. This `opaqueCode` can be used to re-construct the ``ErrorCode/ErrorCode`` that produced it by calling the synthesized ``ErrorCode/ErrorCode/init(opaqueCode:)`` initializer.
 
 Exposing error names or detailed descriptions to the user can be confusing to a non-technical person, and a potential security risk if it describes implementation details. This macro, which provides a default implementation of ``ErrorCode/ErrorCode``, allows the developer to add an opaque error code an existing Swift `enum` type that indicates what went wrong, and where. These types can then be exposed to the user safely using their ``ErrorCode/ErrorCode/opaqueCode`` property. This code can then be reported via analytics, or manually by the user, then used with the ``ErrorCode/ErrorCode/init(opaqueCode:)`` initializer to see exactly which error code was triggered, and potentially even where it was triggered in code.
 
 ### Basic usage
 To use this macro, simply apply it to an `extension` of an enum, and declare a `static` property named `errorCodes` providing an array of instances of `Self` to generate error codes for. Due the limitations of Swift member macros, the `ErrorCode` conformance must be declared manually on the extension:
 
 ```swift
 @ErrorCodeExtension
 extension AppErrorCode: ErrorCode {
 ...
 All of these requirements should be enforced by compiler warnings and errors. If anything doesn't work as expected, please submit an issue.
 */
@attached(member, names: named(OpaqueCode), named(opaqueCode), named(init(opaqueCode: )), named(OpaqueCodeError))
public macro ErrorCodeExtension(codeLength: Int? = nil, codeCharacters: StaticString? = nil) = #externalMacro(module: "ErrorCodeMacros", type: "ErrorCodeMacro")
